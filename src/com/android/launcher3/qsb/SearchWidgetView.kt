...
class SearchWidgetView @JvmOverloads constructor(
    ...
    private lateinit var hotseat: View
    ...
    fun updateColors() {
        ...
        hotseat.background?.setTint(attrColor)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val availableWidth = dp.availableWidthPx - hotseatPadding.left - hotseatPadding.right
        ...
        val qsbWidth = availableWidth - horizontalInset

        val height = if (Utilities.hotseatEnabled(context)) {
            MeasureSpec.getSize(heightMeasureSpec)
        } else {
            0
        }

        val childWidthSpec = MeasureSpec.makeMeasureSpec(qsbWidth, MeasureSpec.EXACTLY)
    ...
    fun getDp(): DeviceProfile {
        return (ActivityContext.lookupContext(context) as ActivityContext).getDeviceProfile()
    }

    fun getTopInset(): Int = if (getDp().isTablet) {
